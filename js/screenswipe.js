...
var swipeHash = window.location.hash.replace(/^#/,'')||swipeCurrent;
console.log('swipeHash: '+swipeHash);
function swipe(target1ID,target2ID,Direction="random",setUrlHash=false){// swipes from a screen to a screen in the direction you say
    if(target1ID==target2ID){}else{
        ...
        t2.style.zIndex=1;
        swipeCurrent = t2.id;
        swipeHash = "#"+swipeCurrent;
        if(setUrlHash===true){
            window.location.hash=swipeHash;
        }
    }
    //console.log("[]->",findScreen().id);
}
function swipeRandom(fromID,setUrlHash=false){// swipes to a random screen from the screen you send
    ...
    swipe(fromID,lst[r].id,eft[e],'random',setUrlHash);
